...
struct Node {
    ...
    Node(unsigned char c_, int weight_, Node *lchild_ = NULL,
         Node *rchild_ = NULL) {
        c = c_;
        weight = weight_;
        lchild = lchild_;
...
Node *create_hfmTree_by_byte_cnt(int const byte_cnt[]) {
    ...
    for (int i = 0; i < 256; i++) {
        if (byte_cnt[i]) { q.push(new Node(i, byte_cnt[i])); }
    }
...
void set_by_bit(unsigned char arr[], int idx, bool value) {
     ...
     * @idx: 下标
     */
    value ? arr[idx / 8] |= (1 << (7 - idx % 8))
          : arr[idx / 8] &= (~(1 << (7 - idx % 8)));
}

void encode_hfmTree(Node const *root, vector<bool> &tree_struct_code,
...
Node *decode_hfmTree(const vector<bool> &tree_struct_code,
                     const vector<unsigned char> &byte_sequence) {
    /**
     * 将01序列和字节序列解码成哈夫曼树
...
void get_encode_table(vector<string> &encode_table, Node *root,
                      string curr_code = "") {
    /**
     * 从哈夫曼树获取编码表
...
vector<unsigned char> zip_process(unsigned char *buf, int file_len) {
     ...
     * 压缩后的格式： 结尾无效位长度（3bit） + 表示树结构的01序列 +
     * 表示叶子节点的byte序列 + 压缩表示的数据 + 填充的无效位
     * @buf: 首地址
    ...
    /* 1. 统计每种字节出现的次数 */
    int byte_cnt[256] = {0};
    for (int i = 0; i < file_len; ++i) { byte_cnt[buf[i]]++; }
    ...
    // log
    cout << "__LINE__ = " << __LINE__ << endl;  // log
    cout << "tree_struct_code.size() = " << tree_struct_code.size()
         << endl;  // log
    for (auto v : tree_struct_code) { cout << v; }
    cout << endl;
    cout << "byte_sequence.size() = " << byte_sequence.size() << endl;  // log
    for (auto v : byte_sequence) { printf("%d ", v); }
    cout << endl;
    ...
    vector<unsigned char> out_buf_vector(out_file_len / 8 +
                                         (out_file_len % 8 != 0));
    ...
    /* 6.1
     * 将代表文件结尾多少位冗余的数字存进缓冲区头部，占3位（冗余位数只有可能是0~7）*/
    /*     类似于大端存储，高位在前，低位在后 */
    ...
    cout << "in line " << __LINE__ << " useless_bit_cnt = " << useless_bit_cnt
         << endl;  // log
    ...
    for (int i = 0; i < byte_sequence.size(); ++i) {
        for (int j = 0; j < 8; ++j) {
            set_by_bit(out_buf_char_star, pointer++,
                       get_by_bit(&byte_sequence[i], j));
        }
    }
...
vector<unsigned char> unzip_process(unsigned char *buf, int file_len) {
    ...
    while (pointer < file_len * 8 - useless_bit_cnt) {
        // cout << "pointer = " << pointer << endl; //log
        // cout << "get_by_bit(buf, pointer) = " << get_by_bit(buf, pointer) <<
        // endl; //log
...
void windows_zip_file(const char *file_in, const char *file_out) {
    ...
    unsigned char ch;
    while (fread(&ch, 1, 1, fp_in)) { buf.push_back(ch); }
...
void windows_unzip_file(const char *file_in, const char *file_out) {
    ...
    FILE *fp_in = fopen(file_in, "rb");
    if (fp_in == NULL) {
        throw "open file " + string(file_in) + " failed in line " +
                to_string(__LINE__);
    }
    ...
    FILE *fp_out = fopen(file_out, "wb");
    if (fp_out == NULL) {
        throw "open file " + string(file_out) + " failed in line " +
                to_string(__LINE__);
    }
...
// return 0;
    ...
    for (int i = 0; i < 1000; ++i) { buf[i] = i * i; }
    ...
    bool right = true;
    for (int i = 0; i < 1000; ++i) { right = right && res[i] == buf[i]; }
